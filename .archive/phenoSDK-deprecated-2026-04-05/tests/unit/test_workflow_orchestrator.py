...
from pheno_workflow.orchestrator import (
    StepStatus,
    TriggerType,
    WorkflowDefinition,
...
async def test_simple_sequential_workflow(orchestrator):
    ...
    assert all(
        r.status == StepStatus.COMPLETED for r in execution.step_results.values()
    )
...
async def test_mixed_parallel_sequential(orchestrator):
    ...
    # Third wave
    workflow.add_step(
        WorkflowStep(step_id="final", handler=simple_handler, dependencies=["seq1"])
    )
...
async def test_async_condition(orchestrator):
    ...
    workflow.add_step(
        WorkflowStep(
            step_id="conditional", handler=simple_handler, condition=async_condition
        ),
    )
...
async def test_step_retry_on_failure(orchestrator):
    ...
    assert (
        execution.step_results["retry_step"].retry_count == 2
    )  # Failed twice, succeeded third
...
async def test_step_failure_continue(orchestrator):
    ...
    workflow.add_step(
        WorkflowStep(
            step_id="after_fail", handler=simple_handler, dependencies=["failing"]
        ),
    )
...
def test_workflow_cycle_detection():
    ...
    workflow.add_step(
        WorkflowStep(step_id="A", handler=simple_handler, dependencies=["C"])
    )
    workflow.add_step(
        WorkflowStep(step_id="B", handler=simple_handler, dependencies=["A"])
    )
    workflow.add_step(
        WorkflowStep(step_id="C", handler=simple_handler, dependencies=["B"])
    )
...
def test_workflow_invalid_dependency():
    ...
    workflow.add_step(
        WorkflowStep(
            step_id="step1", handler=simple_handler, dependencies=["nonexistent"]
        ),
    )
...
def test_workflow_validation_success():
    ...
    workflow.add_step(
        WorkflowStep(step_id="step2", handler=simple_handler, dependencies=["step1"])
    )
...
async def test_list_executions(orchestrator):
    ...
    # Filter by workflow
    workflow1_executions = orchestrator.list_executions(
...
async def test_context_propagation(orchestrator):
    ...
    workflow.add_step(
        WorkflowStep(step_id="step2", handler=step2, dependencies=["step1"])
    )
...
def test_build_simple_workflow():
    ...
    workflow = build_simple_workflow(
        "builder_test",
        [("step1", simple_handler), ("step2", simple_handler)],
    )
...
async def test_complex_workflow_integration(orchestrator):
    ...
    assert all(
        r.status in [StepStatus.COMPLETED] for r in execution.step_results.values()
    )
