...
func GetUserAccessToken(pasetoToken, code string) (models.Git, error) {
	...
	if resp.StatusCode != http.StatusCreated {
		return models.Git{}, fmt.Errorf("failed to get User Access Token: status code %d, body: %s", resp.StatusCode, body)
	}
	...
	response.TokenExpiry = time.Now().Add(7*time.Hour + 45*time.Minute)
	response.RefreshTokenExpiry = time.Now().Add(4*30*24*time.Hour + 12*time.Hour)
