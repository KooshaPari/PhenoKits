...
func GetUserAccessToken( pasetoToken, code string) (models.Git, error) {
	...
	url := fmt.Sprintf("http://github.com/login/oauth/access_token?client_id=%s&client_secret=%s&code=%s", decryptedID, decryptedSecret, code)
	...
	var response models.Git = models.Git{
		...
		TokenExpiry: time.Now(), `json:"expires_in"`
		RefreshTokenExpiry: time.Now(), `json:"refresh_token_expires_in"`
	}
