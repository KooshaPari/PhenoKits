...
func GetUserAccessToken( pasetoToken, code string) (models.Git, error) {
	...
	url := fmt.Sprintf("%s/login/oauth/access_token?client_id=%s&client_secret=%s&code=%s", apiURL, decryptedID, decryptedSecret, code)
	data := url.Values{}
data.Set("client_id", decryptedID)
...
if err != nil {
	...
	err = json.NewDecoder(resp.Body).Decode(&git)
	if err != nil {
		return models.Git{}, fmt.Errorf("failed to parse User Access token response: %v", err)
	}else{
