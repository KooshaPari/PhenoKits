...
func GetUserAccessToken( pasetoToken, code string) (models.Git, error) {
	...
	var secrets models.GitSecret 
	models.DB.First(&secrets)
	decryptedID, err := DecryptSecret(secrets.ClientID)
	if err != nil {
		return models.Git{}, fmt.Errorf("failed to decrypt client id: %v", err)
	}
	decryptedSecret, err := DecryptSecret(secrets.ClientSecret)	
	if err != nil {
	...
	url := fmt.Sprintf("https://github.com/login/oauth/access_token?client_id=%s&client_secret=%s&code=%s", decryptedID, decryptedSecret, code)
	fmt.Println("URL: ", url)
	...
	if resp.StatusCode != http.StatusCreated {
		return models.Git{}, fmt.Errorf("failed to get User Access Token: status code %d details: %s", resp.StatusCode, resp.Body)
	}
