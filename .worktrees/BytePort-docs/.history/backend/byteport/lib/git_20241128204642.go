...
func refreshToken(user models.User, pasetoToken string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("failed to decrypt client secret: %v", err)
			...
			url := fmt.Sprintf("%s/login/oauth/access_token?client_id=%s&client_secret=%s&grant_type=refresh_token&refresh_token=%s", apiURL, ClientID, ClientSecret, decryptedToken)
