...
func refreshToken(user models.User, pasetoToken string) (models.Git, error) {
	...
	fmt.Println("Refreshing Token - Get Secrets: ", decryptedToken)
	fmt.Println("Encrypted Refresh: ", user.Git.RefreshToken) 
	models.DB.First(&secrets)
	clientID,err := DecryptSecret(secrets.ClientID)
	if err != nil {
		return models.Git{}, fmt.Errorf("failed to decrypt client id: %v", err)
	...
	fmt.Printf("client_id: %s\ngrant_type: %s\nrefresh_token: %s\nclient_secret: %s\n", 
		clientID, payload["grant_type"], payload["refresh_token"], clientSecret)
		fmt.Println("User Git Attributes:")
		fmt.Printf("Token: %s\nRefresh Token: %s\nToken Expiry: %v\nRefresh Token Expiry: %v\n",
			user.Git.Token, user.Git.RefreshToken, user.Git.TokenExpiry, user.Git.RefreshTokenExpiry)
      payloadBytes, err := json.Marshal(payload)
    if err != nil {
       return  models.Git{}, fmt.Errorf("failed to marshal payload: %v", err)
    }
