...
type Project struct {
	...
	Deployments []Deployment `gorm:"foreignKey:id;references:UUID"`
}
type Deployment struct {Name     string   `gorm:"type:text"`
		Id       string   `gorm:"type:text;not null"`
		Instance Instance `gorm:"foreignKey:id;references:UUID"`
	}