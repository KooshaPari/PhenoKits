...
func UpdateLink(c *gin.Context ){ 
	...
	decryptedOAI := "err"; var err error
...
func UpdateUser(c *gin.Context){
	...
	if req.LLMConfig.Provider != "" {
		user.LLMConfig.Provider = req.LLMConfig.Provider
	}
	if req.LLMConfig.Providers != nil {
		user.LLMConfig.Providers = req.LLMConfig.Providers
	}
	if req.AwsCreds.AccessKeyID != "" {
		user.AwsCreds.AccessKeyID = req.AwsCreds.AccessKeyID
	}
	if req.AwsCreds.SecretAccessKey != "" {
		user.AwsCreds.SecretAccessKey = req.AwsCreds.SecretAccessKey
	}
	if req.Portfolio.RootEndpoint != "" {
		user.Portfolio.RootEndpoint = req.Portfolio.RootEndpoint
	}
	if req.Portfolio.APIKey != "" {
		user.Portfolio.APIKey = req.Portfolio.APIKey
	}
