...
func HandleCallback(c *gin.Context) {
    ...
    if err := models.DB.Where("git_state = ?", state).First(&user).Error; err != nil {
    ...
    clientID,err := lib.DecryptSecret(user.Git.ClientID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to decrypt client id"})
		return
	}
    clientSecret,err := lib.DecryptSecret(user.Git.ClientSecret)
