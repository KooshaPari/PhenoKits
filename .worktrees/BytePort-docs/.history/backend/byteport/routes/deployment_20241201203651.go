...
func DeployProject(c *gin.Context){
	...
	n, err := io.ReadCloser.Read(jsonProject)
if err != nil {
	c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to read project json"})
}

req.Body = bytes.NewReader(jsonProject[:n])
