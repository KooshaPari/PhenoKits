...
func ValidateLink(c *gin.Context) {
	...
	usergit := (s.tring)lib.DecryptSecret(user.AwsCreds.AccessKeyID)
	fmt.Println("Linking with Github: ", usergit)

	var err error
	err = lib.ValidateAWSCredentials(user.AwsCreds.AccessKeyID,user.AwsCreds.SecretAccessKey)
	if(err != nil){
