...
func ValidateLink(c *gin.Context) {
    // Read the unencrypted credentials from request body
    ...
    err := lib.ValidateAWSCredentials(
        requestUser.AwsCreds.AccessKeyID, 
        requestUser.AwsCreds.SecretAccessKey,
    )
    if err != nil {
    ...
    err = lib.ValidateOpenAICredentials(requestUser.OpenAICreds.APIKey)
    if err != nil {
        c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to validate OAI credentials", "details": err.Error()})
        return
    }

    err = lib.ValidatePortfolioAPI(requestUser.Portfolio.RootEndpoint, requestUser.Portfolio.APIKey)
    if err != nil {
    ...
    encryptedAccessKeyID, err := lib.EncryptSecret(requestUser.AwsCreds.AccessKeyID)
    if err != nil {
        c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to encrypt AWS Access Key ID"})
        return
    }

    encryptedSecretAccessKey, err := lib.EncryptSecret(requestUser.AwsCreds.SecretAccessKey)
    if err != nil {
        c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to encrypt AWS Secret Access Key"})
        return
    }

    encryptedApiKey, err := lib.EncryptSecret(requestUser.OpenAICreds.APIKey)
    if err != nil {
        c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to encrypt OpenAI API Key"})
        return
    }

    encryptedPortfolioURL, err := lib.EncryptSecret(requestUser.Portfolio.RootEndpoint)
    if err != nil {
        c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to encrypt Portfolio Root Endpoint"})
        return
    }

    encryptedPortfolioAPIKey, err := lib.EncryptSecret(requestUser.Portfolio.APIKey)
    if err != nil {
