...
func RetrieveRepositories(c *gin.Context) {
    ...
    repoList, err := lib.ListRepositories(c,decryptedToken)
	if err != nil {
    ...
    c.Data(http.StatusOK, "application/json", c.JSON(repoList)
...
func HandleCallback(c *gin.Context) {
    ...
    authDetails, err := lib.GetUserAccessToken(tokenUrl, authToken, code)
    if err != nil {
    ...
    encryptedTokenExpiry, err := lib.EncryptSecret(authDetails.TokenExpiry)
    if err != nil {
        
        c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to encrypt token expiry"})
        return
    }
    encryptedRefreshTokenExpiry, err := lib.EncryptSecret(authDetails.RefreshTokenExpiry)
    if err != nil {
        c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to encrypt refresh token expiry"})
        return
    }
    user.Git = models.Git{
        Token:              encryptedToken,
        RefreshToken:       encryptedRefreshToken,
        TokenExpiry:        encryptedTokenExpiry,
        RefreshTokenExpiry: encryptedRefreshTokenExpiry,
    }
    models.DB.Save(&user)
