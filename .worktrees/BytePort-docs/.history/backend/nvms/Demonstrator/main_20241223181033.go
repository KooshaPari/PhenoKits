...
1. For complex array fields (screenshots, links, etc.), use a single item following these priority rules:
   - If example data exists in the sample filled template, copy the first item's structure, for referential or date values if not calculable use the template value, else 
