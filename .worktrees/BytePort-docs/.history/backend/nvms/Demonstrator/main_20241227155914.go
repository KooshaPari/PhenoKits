...
func init() {
	spinhttp.Handle(func(w http.ResponseWriter, r *http.Request) {
		...
		filledObject, err := requestFilledTemplate(genRequest, decryptedOAI,user.LLMConfig.Providers[user.LLMConfig.Provider])
...
func requestFilledTemplate(prompt string, key string,modal string) (string, error) {
    response, err := lib.RequestChatCompletion(prompt, key, modal)
    if err != nil {
