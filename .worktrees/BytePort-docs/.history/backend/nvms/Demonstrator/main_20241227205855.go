...
func init() {
	spinhttp.Handle(func(w http.ResponseWriter, r *http.Request) {
		...
		if user.LLMConfig.Provider != "local" { decryptedOAI, err := lib.DecryptSecret(user.LLMConfig.Providers[user.LLMConfig.Provider].APIKey)
		if err != nil {
			fmt.Println("Error decrypting key:", err)
			http.Error(w, "Error decrypting key", http.StatusInternalServerError)
			return
...
Sample API call Provided in response format(Filled Example with Unfilled template in template{} (do not include template attribute in response)):
%s
...
Expected response:  Respond using JSON`
