...
func DeployProject(w http.ResponseWriter, r *http.Request) {
	...
	*/
	...
	accesskey, secretkey, err := lib.DecryptSecret(eAccKey), lib.DecryptSecret(eSecKey)
	if err != nil {
	...
	for _, service := range nvmsConfig.Services {
		...
		DeployAWSService(accesskey,secretkey,service)
		// TODO: Deploy MicroVM
...
func DeployAWSService(accessKey, secretKey string, Service lib.Service) error {
	...
	svc := ec2.New(sess)
	result, err := MakeInstance(svc, ServiceConfigervice.Name, value)
    if err != nil {
