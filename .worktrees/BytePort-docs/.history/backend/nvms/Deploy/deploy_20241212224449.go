...
func processZip (zipBall []byte) (string, error) {

}
func getServiceFolder(name string, zipBall []byte) (string, error) {
	fileMap, err := processZip(zipBall)
}
func DeployAWSService(accessKey, secretKey string, Service lib.Service,zipBall []byte) error {
	serviceFolder, err := getServiceFolder(Service.Name,zipBall)
