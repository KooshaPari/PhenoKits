...
func DeployProject(w http.ResponseWriter, r *http.Request) {
	...
	*/
	...
	if project.Deployments == nil {
        project.Deployments = make(map[string]models.Instance)
    }

    deployID := uuid.New().String()
    project.Deployments[deployID] = models.Instance{
        UUID:            deployID,
        Name:            "main",
        Status:          "initializing",
        Owner:           user.UUID,
        User:           user,
        RootProjectUUID: project.UUID,
        RootProject:     project,
	...
	// append service to deployment
	err = project.BeforeSave(models.dD)
  if instance, ok := project.Deployments[deployID]; ok {
        ...
        project.Deployments[deployID] = instance
    }
	...
	for _, service := range nvmsConfig.Services {
		...
		instance := project.Deployments[deployID]
		instance.Resources = append(project.Deployments[deployID].Resources, models.AWSResource{
		Name: "EC2",
	...
	for _, instance := range ServiceInstances["main"]{
		...
		res  := project.Deployments[deployID]
		res.Resources = append(project.Deployments[deployID].Resources, models.AWSResource{
		Name: "ALBListener",
		ARN: listenArn,
		Status: "deployed",
		Region: "us-east-1", 
	...
	project.Deployments[deployID] = res	
