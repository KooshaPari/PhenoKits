...
func init() {
    spinhttp.Handle(func(w http.ResponseWriter, r *http.Request) {
        ...
        // Create new request for the actual zip file
        zipReq, err := http.NewRequest("GET", redirectURL, nil)
        if err != nil {
            http.Error(w, fmt.Sprintf("Failed to create zip request: %v", err), http.StatusInternalServerError)
            return
        }

        // No need for Authorization header for the redirect URL as it's pre-signed

        // Create new client that follows redirects for the actual download
        downloadClient := &http.Client{}
        zipResp, err := downloadClient.Do(zipReq)
        if err != nil {
