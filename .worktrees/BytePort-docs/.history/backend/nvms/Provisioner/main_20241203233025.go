...
func init() {
    ...
    spinhttp.Handle(func(w http.ResponseWriter, r *http.Request) {
        ...
        zipReader := archives.zip.Reader{
            Data: zipBytes,
        }
        files, err := zipReader.Open()
        if err != nil {
            http.Error(w, err.Error(), http.StatusInternalServerError)
            return
        }
        defer zipReader.Close()
        var nvmsContent, readmeContent string
        // Loop through files to find specific matches
	for _, file := range files {
		// Match file names for *.nvms or README.*
		if strings.HasSuffix(file.Name(), ".nvms") && nvmsContent == "" {
			nvmsContent, err =file.Open()
			if err != nil {
				return "", "", fmt.Errorf("error reading .nvms file: %w", err)
			}
		} else if strings.HasPrefix(strings.ToLower(file.Name()), "readme.") && readmeContent == "" {
			readmeContent, err =file.Open()
			if err != nil {
		...
		if nvmsContent != "" && readmeContent != "" {
			break
		}
	}

        fmt.Println("NVMS File: ", nvmsContent)
        fmt.Println("Readme File: ", readmeContent)
