...
func init() {
    ...
    spinhttp.Handle(func(w http.ResponseWriter, r *http.Request) {
        ...
        err := format.Extract(r,bytes.NewReader(zipBytes), func(ctx context.Context, f archives.FileInfo) error {
            fmt.Println("Extracting file: ", f.Name)
            return nil
        })
        if err != nil {
            http.Error(w, fmt.Sprintf("Failed to extract zip file: %v", err), http.StatusInternalServerError)
        }
        fmt.Println("Extracted zip file")
