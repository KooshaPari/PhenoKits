...
func init() {
    ...
    spinhttp.Handle(func(w http.ResponseWriter, r *http.Request) {
        ...
        fsys, err := archives.FileSystem(nil,"repo.zip", bytes.NewReader(zipBytes))
        if err != nil {
            fmt.Println("Error creating file system: ", err)
            return 
        }
        f, err := fsys.Open("repo.zip")
        if err != nil {
            fmt.Println("Error opening file: ", err)
            return 
        }
        // locate nvms and open
        nvmsFile, err := fsys.Open("repo/test.nvms")
        if err != nil {
            fmt.Println("Error opening nvms file: ", err)
            return
        }
