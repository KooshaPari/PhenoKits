...
func init() {
    spinhttp.Handle(func(w http.ResponseWriter, r *http.Request) {
        ...
        fileMap, err := processZip(zipResp, w,r)
        if err != nil {
            http.Error(w, err.Error(), http.StatusInternalServerError)
            return
        }
        ...
        nvmsFileContent := fileMap[rootDir+"odin.nvms"]
        if nvmsFileContent == nil {
            nvmsFileContent = []byte("No nvms.json found")
        }
...
func processZip(zipResp *http.Response, w http.ResponseWriter, r *http.Request) ( map[string][]byte, error) {
    fmt.Println("Processing zip file...")
...
unc saveFile(file *zip.File, destDir string) error {
    f, err := file.Open()
    if err != nil {
        return err
    }
