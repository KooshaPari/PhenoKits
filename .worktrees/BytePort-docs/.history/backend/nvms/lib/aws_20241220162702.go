...
func DeployEC2(AccessKey string, SecretKey string, bucket S3DeploymentInfo, service models.Service, fileMap map[string][]byte) ([]EC2InstanceInfo,error){
...
func DetectBuildPack(files map[string][]byte, service models.Service) (*models.BuildPack, error) {
    if(service.BuildPack.Name != ""){
        ...
        if service.BuildPack.Name == "" || len(service.BuildPack.Build) == 0 || service.BuildPack.Start == "" || len(service.BuildPack.Packages) == 0 || len(service.BuildPack.P{
