...
func DeployEC2(AccessKey string, SecretKey string, bucket S3DeploymentInfo, service models.Service, fileMap []string) ([]EC2InstanceInfo,error){
	...
	buildScript := generateBuildScript(bucket, service,AccessKey, SecretKey, fileMap)
...
func matchesBuildpackInMemory(files []string, detectFiles []string, servicePath string) bool {
    ...
    for _, file := range detectFiles {
        ...
        for _, f := range files {
            if f == fullPath {
                fmt.Println("Buildpack detected for path: ", fullPath)
                return true
            }
        }
    }
	fmt.Println("No buildpack detected for path: ", normalizedPath)
    return false
}
func generateBuildScript(s3Info S3DeploymentInfo, service models.Service, accessKey, secretKey string, files []string) string {
...
func AwaitInitialization(AccessKey string, SecretKey string, instanceIDs []string) (error){
 ...
 func AnalyzeBuildpackPaths(paths []string) (*BuildpackConfig, error) {
    ...
    // Detect buildpack type
    config := &models.BuildPack{
        RootDir: rootDir,
        Type:    detectBuildpackType(tree),
        Files:   tree,
    }
...
func detectBuildpackType(tree map[string][]string) string {
    ...
    for buildpack, files := range patterns {
        for _, file := range files {
            if containsFile(tree, file) {
                return buildpack
            }
        }
    }
...
func containsFile(tree map[string][]string, filename string) bool {
