...
func matchesBuildpackInMemory(tree map[string]string, detectFiles []string, servicePath string, rootDir string) bool {
    ...
    // Construct the full path including rootDir
    searchPath := rootDir
    if normalizedPath != "" {
        searchPath = rootDir + "/" + normalizedPath
    }
    ...
    for _, file := range detectFiles {
        ...
        if _, exists := tree[fullPath]; exists {
...
func DetectBuildPack(tree map[string]string, service models.Service, rootDir string) (*models.BuildPack, error) {
    ...
    buildpacks := []models.BuildPack{
        // ... your buildpack definitions
    }

    for _, bp := range buildpacks {
        if matchesBuildpackInTree(tree, bp.DetectFiles, service.Path, rootDir) {
            return &bp, nil
        }
    }
