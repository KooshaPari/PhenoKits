...
func ProvisionNetwork(AccessKey string, SecretKey string, projectName string ) ( string, string, string, error) {
	...
	if err != nil {
	 ...
	 loadBalancerArn := albInstance.CreateLoadBalancerResult.LoadBalancers.Member.LoadBalancerArn
	  publicDNS := albInstance.CreateLoadBalancerResult.LoadBalancers.Member.
	...
	return loadBalancerArn, vpcId, publicDNS, nil
