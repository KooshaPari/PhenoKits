...
func RequestChatCompletion(prompt  string, config models.LLM) (string, error) {
    ...
    switch config.Provider {
        ...
        openAI.RequestChatCompletion(reqBody, config.Providers[config.Provider].APIKey)
        case("anthropic"): 
        fmt.Println("Anthropic is not Implemented Yet...")
        case("gemini"):
            fmt.Println("Gemini is not Implemented Yet...")
        case("local"):
            local.RequestChatCompletion(reqBody, config.Providers[config.Provider].APIKey)
}
