...
type NVMS struct {
	...
	Scale      map[string]ScaleConfig    `yaml:"scale,omitempty"`
...
// ClusterConfig defines the configuration for a cluster
type ClusterConfig struct {
	Instances ScaleRange    `yaml:"instances"`
	Path      string        `yaml:"path"`
	Build     BuildCommand  `yaml:"build"`
	Scale     *ScaleRules   `yaml:"scale,omitempty"`
	Resources *ResourceConfig `yaml:"resources,omitempty"`
}

// ScaleRules defines the scaling rules based on various thresholds
type ScaleRules struct {
	...
	ConnectionCount     *uint32  `yaml:"connection_count,omitempty"`
	PlayerCount         *uint32  `yaml:"player_count,omitempty"`
	MessageRate         *uint32  `yaml:"message_rate,omitempty"`
	GPUUtilization      *float32 `yaml:"gpu_utilization,omitempty"`
...
// RDSConfig defines the configuration for AWS RDS
...
// ServiceMode defines the deployment mode of a service
type ServiceMode string

const (
	SingleMode  ServiceMode = "Single"
	ClusterMode ServiceMode = "Cluster"
)

// DynamoDBConfig defines the configuration for AWS DynamoDB
type DynamoDBConfig struct {
	Tables []DynamoDBTable `yaml:"tables"`
}

// DynamoDBTable defines the configuration for a DynamoDB table
type DynamoDBTable struct {
	Name string  `yaml:"name"`
	RCU  *uint32 `yaml:"rcu,omitempty"`
	WCU  *uint32 `yaml:"wcu,omitempty"`
}
...
type LoadBalancerConfig struct {
	LBType LoadBalancerType `yaml:"lb_type"`
	SSL    bool             `yaml:"ssl"`
}
...
const (
	ALB LoadBalancerType = "ALB"
	NLB LoadBalancerType = "NLB"
)

// CDNConfig defines the Content Delivery Network configuration
