...
type BuildPack struct {
    ...
    Packages     []string         `yaml:"PACKAGES",omitempty`      // System packages needed
    PreBuild     []string         `yaml:"PRE_BUILD",omitempty`     // Commands to run before building
    Build        []string         `yaml:"BUILD",omitempty`         // Build commands
    Start        string           `yaml:"START",omitempty`         // Command to start the application
    RuntimeVersions map[string]string `yaml:"RUNTIME_VERSIONS"` // Maps language version files to install commands
    EnvVars      map[string]string   `yaml:"ENV_VARS",omitempty`    // Required environment variables
}
type NVMS struct  {
