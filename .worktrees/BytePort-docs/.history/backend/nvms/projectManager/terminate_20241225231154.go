...
func TerminateProject(w http.ResponseWriter, r *http.Request) {
 ...
 for _, deployment := range deployments {
	for _, resource := range deployment.Resources{
	 terminateResource(w,r,resource, accessKey, sec)}

 }

}
func terminateResource(w http.ResponseWriter, r *http.Request, resource models.AWSResource){
	...
	case "Listener": 
		err := lib.TerminateListener(resource, accessKey, secretKey)
		if err != nil {
			http.Error(w, "Error terminating Listener: "+err.Error(), http.StatusInternalServerError)
			return
		}
