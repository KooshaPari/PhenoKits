...
for len(remainingDeployments) > 0 {
	for deploymentName, deployment := range remainingDeployments {
		...
		for _, resource := range deployment.Resources {
			remainingResources[resource.Name] = resource
		}
		...
		for _, resource := range remainingResources {
			...
			if err != nil {
				...
				if(!strings.Contains(strings.ToLower(string(err.Error())), "does not exist")){deploymentCompleted = false
				termCompleted  = false}
				continue
			}
			if termCompleted {
			fmt.Println("Termination Completed: ", resource.Name)
			delete(remainingResources, resource.Name)
		}
	}
		}
