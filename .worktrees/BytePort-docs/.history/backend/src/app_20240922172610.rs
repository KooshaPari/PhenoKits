...
use diesel::{sqlite::SqliteConnection, migration::Migration}; 
use diesel_migrations::Migration;
use diesel_migrations::MigratorTrait;
...
impl Hooks for App {
    ...
    async fn boot(mode: StartMode, environment: &Environment) -> Result<BootResult> {
        create_app::<Self, dyn MigratorTrait>(mode, environment).await
    }
    ...
    async fn truncate(db: &SqliteConnection) -> Result<()> {
        // Implement your Diesel truncation logic here
        Ok(())
    }

    async fn seed(db: &SqliteConnection, _base: &Path) -> Result<()> {
