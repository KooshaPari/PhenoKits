<script lang="ts">
	...
	function filterRepos() {
		if (!userRepos) userRepos = [];
		filteredRepos = userRepos.filter((repo) =>
			repo.name.toLowerCase().includes(searchQuery.toLowerCase())
		);
		console.log('Filtered repos:', filteredRepos, ' searchQuery:', searchQuery);
	}
		...
		<Command.Group heading="Repositories">
			{#each filteredRepos as repo}
				<Command.Item value={repo.name}>
					<RepoListItem {repo} /></Command.Item
				>
			{/each}</Command.Group
		>
	</Command.List>
