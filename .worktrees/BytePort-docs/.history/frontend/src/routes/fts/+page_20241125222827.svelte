...
async function validateUser() {
    const requestData = {
        Portfolio: { RootEndpoint: "https://example.com", APIKey: "portfolio_key" },
        AwsCreds: { AccessKeyID: "aws_id", SecretAccessKey: "aws_secret" },
        OpenAICreds: { APIKey: "openai_key" }
    };
	...
	async function Link() {
		window.location.href `${SERVER_URL}/link`;
		try {
