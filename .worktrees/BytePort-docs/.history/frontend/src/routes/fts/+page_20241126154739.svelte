<script lang="ts">
	...
	async function checkGitHubLinkStatus() {
        ...
        if (popup.closed) {
            ...
            console.log('Popup closed. Checking GitHub link status...');
            checkGitHubLinkStatus(); // Function to validate the GitHub link
        }
    }, 1000);
    ...
    const popup = window.open(
        `${SERVER_URL}/github/auth`, // Adjust this to your GitHub auth endpoint
        '_blank',
    ...
    const interval = setInterval(async () => {
        ...
        if (popup.closed) {
            ...
            if (backendStatus) {
                ...
                console.error('GitHub linking not completed after popup closed.');
            }
            return;
        }
        ...
        // Check popup content
        try {
            const popupBody = popup.document.body?.innerText; // Access the popup's content
            if (popupBody && popupBody.includes('"message": "GitHub linked successfully"')) {
                console.log('Popup indicates GitHub linking successful!');
                popup.close();
                finalizeGitHubLink();
                clearInterval(interval);
                return;
            ...
            // Ignoring cross-origin issues while waiting for redirection
        }

        // Poll backend for GitHub linking status
        const backendStatus = await checkBackendStatus();
        if (backendStatus) {
            console.log('Backend indicates GitHub linking successful!');
            popup.close();
            finalizeGitHubLink();
            clearInterval(interval);
        }
...
async function finalizeGitHubLink() {
	...
	function Link() {
		popup = window.open(`${SERVER_URL}/link`, '_blank', 'width=600,height=600');
