	...
	async function checkGitHubLinkStatus() {
		...
		const interval = setInterval(async () => {
			...
			if (popup.closed) {
				...
				console.error('Popup closed before completion');
				return;
			}

			try {
				// Check if URL contains callback
				if (popup.location.href.includes(`${SERVER_URL}/api/github/callback`)) {
					const popupBody = popup.document.body?.innerText;
					if (popupBody && popupBody.includes('GitHub linked successfully')) {
						console.log('GitHub linking successful!');
						popup.close();
						await finalizeGitHubLink();
						clearInterval(interval);
					}
				}
			} catch (err) {
				// Ignore cross-origin errors while waiting for callback URL
			}
		}, 1000);
	}
