		...
		return new Promise((resolve) => {
			const interval = setInterval(() => {
				...
				try {
					...
					// above is always about:blank, fix
					
					// Check if we've been redirected back
					if (popup.location.href.includes(SERVER_URL)) {
						const message = popup.document.body?.innerText;
					...
					// Ignore cross-origin errors while waiting for redirect
				}
			}, 500);
		});
	}
