...
<section class="relative h-3/5 w-screen">
	<Dialog.Root>
		<Dialog.Trigger
			...
			<div
				class="active:text-dark-surfaceVariant hover:bg-dark-onSecondaryContainer active:bg-dark-onPrimary bg-dark-primary text-dark-onPrimary m-5 p-5 text-4xl transition-all hover:-translate-y-2 hover:scale-105 active:translate-y-1 active:scale-100"
			>
				<div class="accessBloc align-center flex flex-col justify-center">
					<div class="accessBlocImg h-3/5">
						<img src={'src/assets/img/Byte.png'} alt="project" />
					</div>
					<div class="cardFooting h-2/5">
						<h1 class="mt-3 text-2xl">{project.name}</h1>
					</div>
				</div>
			</div></Dialog.Trigger
		>
