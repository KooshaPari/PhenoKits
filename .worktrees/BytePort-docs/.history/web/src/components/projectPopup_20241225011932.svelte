<script lang="ts">
	...
	onMount(async () => {
		...
		console.log('PROJECTS: ', project.Deployments);
				...
				<div id="resources">
					<Accordion.Root>
						<Accordion.Item value="item-1">
							...
							<Accordion.Content>{#if project.Deployments && project.Deployments.length > 0}
    {#each project.Deployments as resource (resource.ARN)}
        <div id="resource" class="bg-dark-surfaceContainerHigh rounded-lg p-4 mb-2">
            <h1 class="text-xl font-bold text-dark-onSurface">{resource.Name}</h1>
            <h2 class="text-lg text-dark-onSurfaceVariant">Type: {resource.Type}</h2>
            <h3 class="text-md text-dark-secondary">ARN: {resource.ARN}</h3>
            <h3 class="text-md 
                {resource.Status === 'Running' ? 'text-green-500' : 
                 resource.Status === 'Stopped' ? 'text-red-500' : 
                 'text-yellow-500'}">
                Status: {resource.Status}
            </h3>
        </div>
    {/each}
{:else}</Accordion.Content>
						</Accordion.Item>
					</Accordion.Root>
				</div>
			</div>
