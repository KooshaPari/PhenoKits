<script lang="ts">
	...
	onMount(async () => {
		...
		if (project.Deployments) {
			project.Deployments = Object.entries(project.Deployments);
			console.log('SPREAD DEPLOYE: ', project.Deployments);
		}
				...
				<div id="resources">
					<Accordion.Root>
						<Accordion.Item value="item-1">
							...
							<Accordion.Content>
								{#if project.Deployments && Array.from(project.Deployments).length > 0}
									<script>
									...
									{#each project.Deployments as instance}
										...
										{#each instance[1].Resources as resource}
											<script>
