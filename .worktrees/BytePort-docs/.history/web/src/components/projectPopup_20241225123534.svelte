<script lang="ts">
	...
	onMount(async () => {
		...
		deployements = Object.entries(project.Deployments).map(([key, value]) => value);
...
<Dialog.Root>
	<Dialog.Trigger
		...
		<div class="cardFooting">
			<h1>{project.Name}</h1>
			<h2>{Object.entries(project.Deployments).map(([key, value]) => value)[0]?.Resources[0]?.name || 'No deployments'}</h2>
		</div></Dialog.Trigger
	>
