...
<form method="POST" class="space-y-8" use:enhance>
	...
	{#if item == 'integrations'}
		<div class="openAICard">
			...
			<Form.Field name="openai" {form}>
				<Form.Control let:attrs>
					<Form.Label>OpenAI Key</Form.Label>
					<Input class={cn(!($formData.modal.includes("openai")) && 'text-muted-foreground' && 'bg-dark-surfaceContainerLow' && 'border-none')} {...attrs} bind:value={$formData.openai} />
				</Form.Control>
				<Form.FieldErrors />
			</Form.Field>
		</div>
