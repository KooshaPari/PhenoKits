...
<form method="POST" class="space-y-8" use:enhance>
	{#if item == 'integrations'}
		<div class="openAICard align-center flex flex-row gap-3">
			<Form.Field class=" " name="github" {form}>
				<Form.Control let:attrs>
					...
					<Input {...attrs} bind:value={$formData.openai} />
				</Form.Control>
				<Form.FieldErrors />
			</Form.Field>
		...
		<Form.Field class=" " name="portfolio" {form}>
			<Form.Control let:attrs>
				<Form.Label>Portfolio URL</Form.Label>
				<Input {...attrs} bind:value={$formData.op.portfolio.APIKeyenai} />
                <Form.Label>Portfolio Key</Form.Label>
				<Input {...attrs} bind:value={$formData.portfolio.APIKey} />
			</Form.Control>
			<Form.FieldErrors />
		</Form.Field>
	{/if}
