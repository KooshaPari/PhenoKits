<script lang="ts">
	...
	onMount(async () => {
		try {
			...
			const credentials = await getCurrent(user, baseUrl);

			// Update form with fetched data
			const validatedForm = await superValidate(credentials, zod(formSchema));
			mform = superForm(validatedForm, {
				validators: zod(formSchema),
				dataType: 'json',
				onError: ({ result }) => {
					console.error('Form validation failed:', result);
			...
			// Reassign form bindings
			form = mform.form;
			validate = mform.validate;
			enhance = mform.enhance;
		} catch (error) {
