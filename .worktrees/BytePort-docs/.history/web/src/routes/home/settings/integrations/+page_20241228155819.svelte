...
let client: User | null = null;
	...
	const unsubscribe = user.subscribe((value) => {
		...
		if (value.status !== 'authenticated') {
			if (!browser) return;
			console.log('Inner User unauthenticated, redirecting...');
			goto('/login');
		} else {
			...
			if (!initialized) {
				...
				getBaseUrl().then((basUrl) => {
					baseUrl = basUrl;
					console.log('Promise_Set Base URL:', baseUrl);
					initializeUser(baseUrl);

					getCurrent(client, baseUrl).then((data) => {
						console.log('Data:', data);
						resp = data;
						superValidate(resp, zod(formSchema)).then((data) => {
	...
	async function getCurrent(user: User | null, baseUrl: string): Promise<UserCreds> {
		console.log('Pulling current creds');
		if (!user || user.UUID == '') {
			console.log('No user');
			return {
				github: '',
			...
			if (response.ok) {
				...
				return {
					...
					llm: { provider: resp.LLM.provider, providers: resp.LLM.Providers },
	...
	onMount(() => {
		...
		getBaseUrl().then((basUrl) => {
			...
			console.log('Promise_Set Base URL:', baseUrl);
			initializeUser(baseUrl);
			getCurrent(client, baseUrl).then((dataa) => {
				console.log('Data:', dataa);
				let resp = dataa;
			});
		});

		//nform = await superValidate(resp, zod(formSchema));
		//console.log('Form:', nform);
		// Ensure the user initialization is complete

		//return unsubscribe;
	});
	let mform: SuperForm<FormSchema>;
	let nform: SuperValidated<FormSchema>;
	let baseUrl = '';
	let resp: UserCreds = {
		github: '',
		aws: { accessKey: '', secretKey: '' },
