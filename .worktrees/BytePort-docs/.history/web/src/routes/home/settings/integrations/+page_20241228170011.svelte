						...
						<Popover.Root>
							<Form.Control let:attrs>
								...
								<Popover.Trigger
									...
									{...attrs}
								>
									{providers.find((provider) => provider.value === $form.provider)?.label || 'Select a provider'}
									<CaretSort class="ml-2 size-4 shrink-0 opacity-50" />
								</Popover.Trigger>
								<input hidden value={!$form.llm.providers[.get]($form.llm.provider)?.modal} name={attrs.name} />
							</Form.Control>
							<Popover.Content class="w-[200px] p-0">
								<Command.Root>
									<Command.List>
