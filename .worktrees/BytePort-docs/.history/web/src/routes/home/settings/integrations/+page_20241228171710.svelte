<script lang="ts">
	...
	const modals = [
		...
		{ label: 'GPT-4o', provider: 'gemini', value: 'gpt-4o' },
		{ label: 'GPT-4o-mini', provider: 'gemini', value: 'gpt-4o-mini' },
		{ label: 'GPT-o1', provider: 'gemini', value: 'gpt-o1' },

		{ label: 'GPT-4o', provider: 'anthropic', value: '3.5-sonnet' },
		{ label: 'GPT-4o-mini', provider: 'anthropic', value: 'gpt-4o-mini' },
		{ label: 'GPT-o1', provider: 'anthropic', value: 'gpt-o1' },
	...
	async function getCurrent(user: User, baseUrl: string): Promise<UserCreds> {
		...
		try {
			...
			return {
				github: resp.git.Token.includes('') ? 'Not Linked' : 'Authenticated',
				aws: {
					accessKey: resp.aws_creds.AccessKeyID,
					secretKey: resp.aws_creds.SecretAccessKey
				},
				llm: {
					...
					<Form.Field form={mform} name="llm" class="flex flex-col justify-center">
						<Popover.Root>
							<Form.Control let:attrs>
								...
								<input hidden value={$form.llm.provider} name={attrs.name} />
							</Form.Control>
							...
							<Form.Control let:attrs>
								...
								<Popover.Trigger
									...
									class={cn(
										buttonVariants({ variant: 'outline' }),
										'w-[200px] justify-between',
										!$form.llm.providers.get($form.llm.provider)?.modal && 'text-muted-foreground'
									)}
									{...attrs}
								>
									{providers.find((provider) => provider.value === $form.provider)?.label ||
										'Select a provider'}
									<CaretSort class="ml-2 size-4 shrink-0 opacity-50" />
								</Popover.Trigger>
								<input
									hidden
									value={$form.llm.providers[$form.llm.provider].modal}
									name={attrs.name}
								/>
							</Form.Control>
							<Popover.Content class="w-[200px] p-0">
						...
						{#if $form.llm.provider === 'local'}
							<Form.Control let:attrs>
								<Form.Label>OpenAI Key</Form.Label>
								<Input
									class={cn(
										!$form.llm.provider.includes('openai') && [
											'text-muted-foreground',
											'bg-muted',
											'border-dark-surfaceContainerLow'
										]
									)}
									disabled={$form.llm.provider.includes('local')}
									{...attrs}
									bind:value={$form.llm.providers[$form.llm.provider].modal}
								/>
							</Form.Control>-->
						<Form.FieldErrors />
					</Form.Field>
