							...
							<Form.Control let:attrs>
								...
								<Popover.Trigger
									...
									class={cn(
										buttonVariants({ variant: 'outline' }),
										'w-[200px] justify-between',
										!$form.llm.providers[$form.llm.provider]?.modal && 'text-muted-foreground'
									)}
									{...attrs}
								>
									{modals.find(
										(modal) => modal.value === $form.llm.providers[$form.llm.provider]?.modal
							...
							<Popover.Content class="w-[200px] p-0">
								<Command.Root>
									<Command.List>
										...
										{#if modal.provider == $form.llm.providers[$form.llm.provider].modal
											<Command.Item
												{...form}
												value={modal.label}
												onSelect={() => {
											...
											>
												<Check
													class={cn(
														'mr-2 size-4',
														modal.value === $form.llm.providers[$form.llm.provider].modal ? 'opacity-100' : 'opacity-0'
													)}
												/>
												{modal.label}
											</Command.Item>
