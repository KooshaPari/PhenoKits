<script lang="ts">
	...
	const DEFAULT_CREDS: UserCreds = {
		...
		llm: { provider: '', providers: new Record<string, AIProvider>() },
	...
	type UserCreds = {
		github: string;
		aws: { accessKey: string; secretKey: string };
		llm: { provider: string; providers: Record<string, AIProvider> };
		demo: { endpoint: string; apiKey: string };
	};
					...
					<Form.Field form={mform} name="llm" class="flex flex-col justify-center">
						...
						<Popover.Root>
							<Form.Control let:attrs>
								...
								>
									{modals.find(
										(modal) => modal.value === $form.llm.providers.get($form.llm.provider)?.modal
									)?.label || 'Select a Modal'}
									<CaretSort class="ml-2 size-4 shrink-0 opacity-50" />
								</Popover.Trigger>
								<input
									hidden
									value={$form.llm.providers.get($form.llm.provider)?.modal}
									name={attrs.name}
								/>
							</Form.Control>
							<Popover.Content class="w-[200px] p-0">
								<Command.Root>
									<Command.List>
										{#each modals as modal}
											{#if modal.provider === ($form.llm.providers[$form.llm.provider].modal as string)}
												<Command.Item
													{...form}
													value={modal.label}
													onSelect={() => {
