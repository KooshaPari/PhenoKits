<script lang="ts">
	...
	const unsubscribe = user.subscribe(async (value) => {
		...
		if (client) {
			...
			for provider of userCreds.llm.providers {
				if (!modals.find((modal) => modal.value === provider)) {
					modals.push({ label: provider, provider: 'local', value: provider });
				}
			}
