<script lang="ts">
	...
	const unsubscribe = user.subscribe(async (value) => {
		...
		if (client) {
			...
			if (!userCreds.llm.providers) {
				userCreds.llm.providers = {};
			}
			for (const [provider, providerData] of Object.entries(userCreds.llm.providers)) {
				if (!providerData) {
					userCreds.llm.providers[provider] = {
						modal: '',
						apiKey: ''
					};
				}
			...
			// Also ensure all provider types exist
			for (const provider of providers) {
				if (!userCreds.llm.providers[provider.value]) {
					console.log('Missing:', provider.value);
					userCreds.llm.providers[provider.value] = {
						modal: '',
						apiKey: ''
					};
				}
