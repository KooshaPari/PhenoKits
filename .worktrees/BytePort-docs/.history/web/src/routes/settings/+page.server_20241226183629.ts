...
async function getCurrent(user: User | null, baseUrl: string): Promise<FormSchema> {
	if (!user) {
		return {
			github: '',
        ...
        if (response.ok) {
            ...
            return {
                github: resp.github,
                aws: { accessKey: resp.aws.accessKey, secretKey: resp.aws.secretKey },
                openai: resp.openai,
                demo: { endpoint: resp.demo.endpoint, apiKey: resp.demo.apiKey },
                modal: resp.modal
            };
        }
