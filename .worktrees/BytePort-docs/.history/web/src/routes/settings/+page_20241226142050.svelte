<script lang="ts">
	...
	import IntegrationSettings from '../../components/integrationSet.svelte';
	import { populateLists } from '$lib/utils';
	const settingItemsMap: Map<string, { component: any; props: any }> = new Map<string, { component: any; props: any }>([
		//['Profile', { component: ProfileSettings, props: {} }],
		['Integrations', { component: IntegrationSettings, props: {} }]
	]);
...
<div class="bg-dark-surface flex h-screen w-screen overflow-x-hidden" id="mainDashPar">
	...
	<div id="body" class="w-4/5">
		...
		<div id="mainBody">
			{#each in [...settingItemsMap] as [key, value]}
				<Button.Root  >{key}</Button.Root>
			{/each}
		</div>
		<div id="footer"></div>
	</div>
