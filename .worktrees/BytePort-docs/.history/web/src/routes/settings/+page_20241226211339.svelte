<script lang="ts">
	...
	import SetComp fr SuperValidated<{ github: string; aws: { accessKey: string; secretKey: string; }; openai: string; demo: { endpoint: string; apiKey: string; }; modal: \"openai\" | \"local\"; }, any, { github: string; aws: { accessKey: string; secretKey: string; }; openai: string; demo: { ...; }; modal: \"openai\" | \"local\"; }> om '$lib/../components/settings.svelte';
	export let form: ;
	...
	onMount(() => {
		getBaseUrl().then(async (url) => {
			...
			const newR = await superValidate(zod(formSchema));
			form = newR;
		});
		return () => {
			unsubscribe();
...
<div class="bg-dark-surface flex h-screen w-screen overflow-x-hidden" id="mainDashPar">
	...
	<div id="body" class="w-4/5">
		...
		<div id="mainBody">
			<SetComp id="bodyCont" item={'integrations'} {form} {baseUrl} {data} />
		</div>
		<div id="footer"></div>
	</div>
</div>
