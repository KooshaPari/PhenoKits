...
func createALBListener(AccessKey string, SecretKey string, projectName string, loadBalancerArn string, vpcId string, instanceId string, port string  ) (string, error ){
	albClient, err := awsnet.NewALB(aws.Config{
		AccessKeyId: AccessKey,
		SecretAccessKey: SecretKey,
		SessionToken: "",
	...
	targetArn, err := registerService(AccessKey, SecretKey, loadBalancerArn, projectName, projectName, vpcId, instanceId, port)
