...
func downloadAndPushToS3(archiveURL, AccessKey, SecretKey, ProjectName string) (S3DeploymentInfo, error) {
    // First get the zip data
    zipBall, err := DownloadAndUploadRepo(archiveURL)
    if err != nil {
        return S3DeploymentInfo{}, fmt.Errorf("download failed: %v", err)
    }

    // Then proceed with S3 upload
    fmt.Println("Uploading to S3...")
    cfg := aws.Config{
        AccessKeyId:     AccessKey,
        SecretAccessKey: SecretKey,
    ...
    s3Client, err := s3.NewS3(cfg)
    if err != nil {
        return S3DeploymentInfo{}, fmt.Errorf("s3 client creation failed: %v", err)
    }
    fmt.Println("Created S3 client")
    ...
    err = s3Client.CreateBucket(ctx, bucketName)
    if err != nil {
        return S3DeploymentInfo{}, fmt.Errorf("bucket creation failed: %v", err)
    }
    fmt.Println("Created bucket")

    err = s3Client.PutObject(ctx, bucketName, "src.zip", zipBall)
    if err != nil {
        return S3DeploymentInfo{}, fmt.Errorf("upload failed: %v", err)
    }
    fmt.Println("Uploaded to S3")
