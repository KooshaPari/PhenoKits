...
func (c *Client) PutObject(ctx context.Context, bucketName, objectName string, dataPath string) error {
	// send file at filepathto s3
	data, err := io.ReadAll(dataPath)
	if err != nil {
		return fmt.Errorf("failed to read file: %w", err)
	}
	req, err := c.newRequest(ctx, http.MethodPut, bucketName, objectName, nil)
	if err != nil {
		return err
	}
