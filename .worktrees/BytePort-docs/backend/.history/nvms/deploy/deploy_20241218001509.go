...
func DeployProject(w http.ResponseWriter, r *http.Request) {
	...
	*/
	...
	bucketName, codebaseName, err := pushToS3(codebase, accesskey, secretkey, project.Name)
	fmt.Println("Bucket Name: ", codebaseName)
	if err != nil {
		fmt.Println("Error pushing to S3: ", err)
		http.Error(w, "Error pushing to S3: "+err.Error(), http.StatusInternalServerError)
		return
	}
	err = deployEC2(accesskey, secretkey, bucketName, project.NvmsConfig.Services[0])
	if err != nil {
