...
func DeployProject(w http.ResponseWriter, r *http.Request) {
	...
	*/
	...
	for _, service := range nvmsConfig.Services {
		...
		fmt.Println("Serve")
		instances, err := DeployNVMSService(accesskey, secretkey, bucket.BucketName, service)
		if err != nil {
...
func DeployNVMSService(AccessKey string, SecretKey string, BucketName string, service models.Service) ([]EC2InstanceInfo, error) {
	instances, err := deployEC2(AccessKey, SecretKey, BucketName, service)
	if err != nil {
		fmt.Println("Error deploying EC2: ", err)
		http.Error(w, "Error deploying EC2: "+err.Error(), http.StatusInternalServerError)
		return
	}
