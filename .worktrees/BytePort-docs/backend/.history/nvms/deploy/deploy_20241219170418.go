...
func DeployProject(w http.ResponseWriter, r *http.Request) {
	...
	*/
	...
	projectEndpoint, zoneID, err := provisionNetwork(accesskey, secretkey, project.Name)
	if err != nil {
	...
	for _, service := range nvmsConfig.Services {
		...
		fmt.Println("Serve")
		instances, err := DeployNVMSService(accesskey, secretkey, bucket, service, projectEndpoint)
		if err != nil {
...
func DeployNVMSService(AccessKey string, SecretKey string, Bucket S3DeploymentInfo, service models.Service, projectEndpoint string) ([]EC2InstanceInfo, error) {
	instances, err := deployEC2(AccessKey, SecretKey, Bucket, service, projectEndpoint)
	if err != nil {
