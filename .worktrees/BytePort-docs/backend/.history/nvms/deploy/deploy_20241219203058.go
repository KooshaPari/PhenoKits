...
func DeployProject(w http.ResponseWriter, r *http.Request) {
	...
	*/
	...
	// write service Instances to body
	// run a for loop on service instances, for each await deploy then register services, prov network and finally listener rules.
	for _, instances := range ServiceInstances {
		for _, instance := range instances {
			err := AwaitIntialization(accesskey, secretkey, instance.InstanceID)
	...
	w.Write(servBody)}
	fmt.Println("Intialized: ", instances)
	}
}
