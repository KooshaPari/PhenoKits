...
func DeployProject(w http.ResponseWriter, r *http.Request) {
	...
	*/
	...
	servBody, err := json.Marshal(ServiceInstances)
	...
	err := awaitInitialization(accesskey, secretkey, instIDs)
	...
	lbArn, vpcId, err := provisionNetwork(accesskey, secretkey, project.Name )
	if err != nil {
	...
	listenArn, err := createALBListener(accesskey, secretkey, project.Name, lbArn )
	fmt.Println("Completed Deploy.")
	for i, instances := range ServiceInstances {
		service := nvmsConfig.Services[i]
		port := nvmsConfig.Services[i].Port
		for _, instance := range instances {
			
		tgArn, err := registerService(accesskey, secretkey, lbArn, project.Name, name, vpcId, instance.InstanceId, port)
		if err != nil {
			http.Error(w, "Error registering service: "+err.Error(), http.StatusInternalServerError)
			return
		}
		err = setListenerRules(accesskey, secretkey, listenArn, tgArn, name)
		if err != nil {
...
/*
for _, instance := range instances {
			fmt.Println("Adding Record: ", instance)
			//addNewRecord(accesskey, secretkey, service.Name,zoneID, project.Name, instance.PrivateIP)
			zoneID, err := provisionNetwork(accesskey, secretkey, project.Name)
	if err != nil {
		fmt.Println("Error provisioning network", err)
		http.Error(w, "Error provisioning network: "+err.Error(), http.StatusInternalServerError)
		return
