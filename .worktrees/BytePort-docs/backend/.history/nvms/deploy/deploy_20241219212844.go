...
func DeployProject(w http.ResponseWriter, r *http.Request) {
	...
	*/
	...
	var serviceMap map[string]models.Service 
	...
	for name, instances := range ServiceInstances {
		...
		tgArn, err := registerService(accesskey, secretkey, lbArn, project.Name, name, vpcId, instance.InstanceID, strings.tpservice.Port.toString())
		if err != nil {
