...
func DeployProject(w http.ResponseWriter, r *http.Request) {
	...
	*/
	...
	fmt.Println("S`ervice Instances: ", ServiceInstances)
	...
	listenArn, err := createALBListener(accesskey, secretkey, project.Name, lbArn, vpcId  )
	...
	for name, instances := range ServiceInstances {
		...
		tgArn, err := registerService(accesskey, secretkey, lbArn, project.Name, name, vpcId, instance.InstanceID, string(service.Port))
		if err != nil {
