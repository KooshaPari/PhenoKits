...
func parseNVMSConfig(yamlContent string) (*models.NVMS, error) {
    ...
    for name, svc := range config.Services {
		...
		if name == "main"{
			if(found){
				return nil, fmt.Errorf("service %s already defined", name)
			}
		}
    }
...
func DeployProject(w http.ResponseWriter, r *http.Request) {
	...
	*/
	...
	for _, instance := range ServiceInstances["main"]{
		...
		listenArn, err = createALBListener(accesskey, secretkey, project.Name, lbArn, vpcId, instance.InstanceID,  serviceMap["main"].Port)
	...
	for name, instances := range ServiceInstances {
		...
		tgArn, err := registerService(accesskey, secretkey, lbArn, project.Name, name, vpcId, instance.InstanceID,   service.Port )
		if err != nil {
		...
		err = setListenerRules(accesskey, secretkey, listenArn, tgArn, name, priority)
		if err != nil {
