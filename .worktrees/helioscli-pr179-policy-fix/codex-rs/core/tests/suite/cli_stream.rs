...
fn codex_bin() -> std::path::PathBuf {
    ...
    let manifest_path = workspace_root.join("codex-rs/cli/Cargo.toml");
    let output = std::process::Command::new("cargo")
        ...
        .arg("codex")
        .output()
        .unwrap_or_else(|err| panic!("failed to build codex binary: {err}"));
    assert!(
        output.status.success(),
        "failed to build codex binary: {}",
        String::from_utf8_lossy(&output.stderr)
    );

    let bin = workspace_root.join("codex-rs/target/debug/codex");
    assert!(bin.is_file(), "codex binary missing at {}", bin.display());
    bin
}
