...
fn assert_message_role(request_body: &serde_json::Value, role: &str) {
    let Some(actual_role) = request_body["role"].as_str() else {
        panic!("role should be a string");
    };
...
fn write_auth_json(
    ...
    let header_b64 = b64(
        &serde_json::to_vec(&header).unwrap_or_else(|err| panic!("header should serialize: {err}")),
    );
    let payload_b64 = b64(
        &serde_json::to_vec(&payload)
            .unwrap_or_else(|err| panic!("payload should serialize: {err}")),
    );
    ...
    std::fs::write(
        ...
        serde_json::to_string_pretty(&auth_json)
            .unwrap_or_else(|err| panic!("auth json should serialize: {err}")),
    )
    .unwrap_or_else(|err| panic!("auth json write should succeed: {err}"));
