...
async fn build_test_codex(server: &wiremock::MockServer) -> Result<TestCodex> {
    ...
    let default_config = core_test_support::load_default_config_for_test(&temp_home).await;
    let mut model_info =
        codex_core::test_support::construct_model_info_offline(MODEL_WITH_TOOL, &default_config);
    model_info.experimental_supported_tools = vec!["grep_files".to_string()];
    let cache_models = vec![model_info];

    let mut builder = test_codex()
        .with_model(MODEL_WITH_TOOL)
        .with_pre_build_hook(move |home| write_models_cache(home, &cache_models));
    builder.build(server).await
}

fn write_models_cache(home: &std::path::Path, models: &[ModelInfo]) {
    ...
    let bytes = serde_json::to_vec_pretty(&cache)
        .unwrap_or_else(|err| panic!("models cache should serialize: {err}"));
    std::fs::write(&cache_path, bytes)
        .unwrap_or_else(|err| panic!("models cache write should succeed: {err}"));
}
