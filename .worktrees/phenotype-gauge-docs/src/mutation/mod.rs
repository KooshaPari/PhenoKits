...
impl MutationTracker {
    ...
    pub fn record_line_execution(&mut self, file: &str, line: usize) {
        let _ = line;
        self.files.entry(file.to_string()).or_default().lines_executed += 1;
    }
    ...
    pub fn introduce_mutation(&mut self, file: &str, line: usize, kind: MutationKind) -> String {
        ...
        self.files.entry(file.to_string()).or_default().mutations.push(Mutation {
            id: id.clone(),
            line,
...
impl CoverageReport {
    ...
    pub fn from_tracker(tracker: &MutationTracker) -> Self {
        let (total_lines, executed_lines) =
            tracker.files().fold((0, 0), |(t, e), (_, exec)| (t + 100, e + exec)); // TODO: actual LOC
        Self {
            total_lines,
            executed_lines,
...
mod tests {
    ...
    fn test_record_line_execution() {
        ...
        assert!(tracker.coverage("src/lib.rs") > 0.0); // TODO: actual LOC
    }
