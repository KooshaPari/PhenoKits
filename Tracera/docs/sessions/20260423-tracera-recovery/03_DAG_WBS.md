...
| R10 | In progress | Burn down strict web type-aware lint production backlog | Ten production tranches are warning-only under targeted oxlint and `typecheck:web` passes; full strict count reduced from 2,503 to 1,897 |

## Next Queue

1. Continue production-first strict type-aware lint tranches, now led by three-error
   clusters in app providers, `useChat.ts`, enterprise UI, temporal/layout components,
   remaining graph components, equivalence import/export wizards, API docs wrappers,
   and `api/system.ts`. Keep colocated `lib/websocket.test.ts` and `lib/csrf.test.ts`
   queued after production source.
