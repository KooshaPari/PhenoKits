...
| R10 | In progress | Burn down strict web type-aware lint production backlog | Seven production tranches are warning-only under targeted oxlint and `typecheck:web` passes; full strict count reduced from 2,503 to 2,035 |

## Next Queue

1. Continue production-first strict type-aware lint tranches, currently led by
   four-error clusters in `GraphView.tsx`, `FeatureListView.tsx`, `ADRDetailView.tsx`,
   `auth-store.ts`, project test-suite/test-case views, websocket/cache/resilience
   utilities, and graph equivalence import/export utilities.
