...
- `bun run lint:type-aware:web:strict` still exits 1, but the strict-error count is
  reduced from 2,503 to 1,897 after ten production tranches. The current largest
  remaining production clusters are at three errors per file, led by `test/setup.ts`,
  `app-providers.jsx`, `useChat.ts`, enterprise UI components, temporal components,
  `Breadcrumb.tsx`, remaining graph components, equivalence import/export wizards,
  API docs wrappers, and `api/system.ts`. Colocated `lib/websocket.test.ts` and
  `lib/csrf.test.ts` remain larger test clusters outside `__tests__/`.
