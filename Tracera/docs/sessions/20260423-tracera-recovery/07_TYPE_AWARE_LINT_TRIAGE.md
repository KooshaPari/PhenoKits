...
The eleventh production tranche is complete:

- `app-providers.jsx`, `useChat.ts`, and `api/system.ts` pass targeted strict
  type-aware oxlint.
- `enterprise-table.tsx`, `enterprise-button.tsx`, and `Breadcrumb.tsx` pass targeted
  strict type-aware oxlint.
- `DiffViewer.tsx`, `UnifiedGraphView.tsx`, and `ThumbnailPreview.tsx` pass targeted
  strict type-aware oxlint.
- `PageDecompositionView.tsx`, `GraphSearch.tsx`, `CrossPerspectiveSearch.tsx`, and
  `ComponentUsageMatrix.tsx` pass targeted strict type-aware oxlint.
- `ImportWizard.tsx`, `ExportWizard.tsx`, and `swagger-ui-wrapper.tsx` pass targeted
  strict type-aware oxlint.
- `bun run typecheck:web` passes after the tranche.

The twelfth production tranche is complete:

- `enhancedRenderers.ts`, `ExpandableNode.tsx`, and `graphLayout.worker.ts` pass
  targeted strict type-aware oxlint.
- `HybridGraphView.tsx` and `HybridGraphView.enhanced.tsx` pass targeted strict
  type-aware oxlint.
- `TraceabilityMatrixView.tsx`, `ImpactAnalysisView.tsx`, `FeatureDetailView.tsx`,
  and `DashboardView.tsx` pass targeted strict type-aware oxlint.
- `edgeAggregation.ts`, `diff-export.ts`, `csrf.ts`, `ServiceWorkerCache.ts`, and
  `MemoryCache.ts` pass targeted strict type-aware oxlint.
