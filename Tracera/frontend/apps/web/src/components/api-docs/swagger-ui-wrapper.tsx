...
type SwaggerRequestInterceptor = NonNullable<SwaggerUIProps['requestInterceptor']>;

type SwaggerResponseInterceptor = NonNullable<SwaggerUIProps['responseInterceptor']>;

type SwaggerRequest = Parameters<SwaggerRequestInterceptor>[0];

type SwaggerResponse = Parameters<SwaggerResponseInterceptor>[0];

type SwaggerRequestResult = ReturnType<SwaggerRequestInterceptor>;

type SwaggerResponseResult = ReturnType<SwaggerResponseInterceptor>;

type SwaggerHeaderBag = {
  set: (name: string, value: string) => void;
};

type NormalizedSwaggerProps = Required<
  Pick<
...
const isHeaderBag = (value: unknown): value is SwaggerHeaderBag =>
  isObject(value) && 'set' in value && typeof value.set === 'function';
...
const fetchSpecFromUrl = async (specUrl?: string): Promise<object | null> => {
  if (specUrl === undefined || specUrl === '') {
    return null;
  }
...
const setHeader = (req: SwaggerRequest, name: string, value: string): void => {
  const { headers } = req;
...
const normalizeSwaggerProps = (props: SwaggerUIWrapperProps): NormalizedSwaggerProps => ({
  ...SWAGGER_DEFAULTS,
  ...props,
});

const useSpecData = (specUrl?: string, spec?: object): object | null => {
  ...
  useEffect(() => {
    ...
    fetch(specUrl)
      ...
      .then((data: unknown) => {
        setSpecData(isObject(data) ? data : null);
        return undefined;
      })
      .catch((error: unknown) => {
...
const useCopySpecUrl = (specUrl?: string): { copied: boolean; copySpecUrl: () => Promise<void> } => {
...
const useDownloadSpec = (resolvedSpec: object | null, specUrl?: string): (() => Promise<void>) =>
  useCallback(async () => {
...
const useSwaggerInterceptors = (
  ...
  defaultRequestInterceptor: SwaggerRequestInterceptor;
  defaultResponseInterceptor: SwaggerResponseInterceptor;
} => {
  const defaultRequestInterceptor = useCallback(
    (req: SwaggerRequest): SwaggerRequestResult => {
  ...
  const defaultResponseInterceptor = useCallback(
    (res: SwaggerResponse): SwaggerResponseResult => {
...
const useSwaggerProps = (params: {
  ...
  useMemo(
    () => {
      ...
      if (params.resolvedSpec !== null) {
        swaggerProps.spec = params.resolvedSpec;
      } else if (params.specUrl !== undefined && params.specUrl !== '') {
        swaggerProps.url = params.specUrl;
      }
      return swaggerProps;
    },
    [
...
const SwaggerContent = ({
  ...
  if (hasSpec) {
    return <SwaggerUI {...swaggerProps} />;
  }
  return <LoadingState label='Loading API Documentation...' />;
};
...
export const SwaggerUIWrapper = (props: SwaggerUIWrapperProps): ReactElement => {
