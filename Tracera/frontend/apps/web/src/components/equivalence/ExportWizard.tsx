...
const isExportFormat = (value: string): value is ExportFormat => value === 'json' || value === 'yaml';
...
const parseExportStats = (data: unknown): ExportStats | null => {
  if (!isRecord(data)) {
    return null;
  }
  return {
    averageConfidence: readNumber(data, 'averageConfidence'),
    concepts: readNumber(data, 'concepts'),
    links: readNumber(data, 'links'),
    perspectives: readNumber(data, 'perspectives'),
    projections: readNumber(data, 'projections'),
  };
};
...
const fetchExportStats = async (projectId: string): Promise<ExportStats> => {
  ...
  const stats = parseExportStats(data);
  if (stats === null) {
    throw new Error('Failed to fetch statistics');
  }
  return stats;
...
const OptionsStep: FC<OptionsStepProps> = ({
  ...
  <div className='space-y-6'>
    ...
    {stats !== null && <ExportSummary stats={stats} />}
