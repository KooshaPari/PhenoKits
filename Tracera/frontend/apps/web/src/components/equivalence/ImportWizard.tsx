...
const readValidationItems = (value: unknown): ValidationError[] => {
...
const readConflicts = (value: unknown): ValidationResult['conflicts'] => {
  if (!Array.isArray(value)) {
    return undefined;
  }
  return value.flatMap((item) => {
    ...
    return [{ message, severity, type }];
  });
};

const parseValidationResult = (data: unknown): ValidationResult | null => {
  if (!isRecord(data)) {
    return null;
  }
  const summary = isRecord(data['summary']) ? data['summary'] : {};
  const result: ValidationResult = {
    errors: readValidationItems(data['errors']),
    summary: {
      concepts: readNumber(summary, 'concepts'),
      links: readNumber(summary, 'links'),
      projections: readNumber(summary, 'projections'),
    },
    valid: data['valid'] === true,
    warnings: readValidationItems(data['warnings']),
  };
  const conflicts = readConflicts(data['conflicts']);
...
const parseImportResult = (data: unknown): ImportResult | null => {
  if (!isRecord(data)) {
    return null;
  }
  const errors = data['errors'];
  return {
    concepts_imported: readNumber(data, 'concepts_imported'),
    errors: Array.isArray(errors) ? errors : undefined,
    links_imported: readNumber(data, 'links_imported'),
    projections_imported: readNumber(data, 'projections_imported'),
    status: readString(data, 'status'),
    summary: readString(data, 'summary'),
  };
};

const readValidationResult = async (response: Response): Promise<ValidationResult> => {
  const data: unknown = await response.json();
  if (!response.ok) {
    throw new Error(parseErrorMessage(data, 'Validation failed'));
  }
  const result = parseValidationResult(data);
  if (result === null) {
...
const readImportResult = async (response: Response): Promise<ImportResult> => {
  const data: unknown = await response.json();
  if (!response.ok) {
...
const isConflictStrategy = (value: string): value is ConflictStrategy =>
  value === 'skip' || value === 'replace' || value === 'merge';
...
const validateImportFile = async (projectId: string, file: File): Promise<ValidationResult> => {
  ...
  return readValidationResult(response);
};
...
const createHandleProceedToConflicts =
  (validation: ValidationResult | null, setStep: SetStep) => () => {
    if ((validation?.conflicts?.length ?? 0) > 0) {
      setStep('conflicts');
    } else {
      setStep('confirm');
    }
...
const UploadStep: FC<UploadStepProps> = ({ error, file, onFileSelect }) => (
  <div className='space-y-4'>
    ...
    {file !== null && <SelectedFile file={file} />}
    {error !== null && error !== '' && <ErrorAlert message={error} />}
  </div>
);
