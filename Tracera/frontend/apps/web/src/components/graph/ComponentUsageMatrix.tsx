...
const COMPONENT_CATEGORY_VALUES = new Set<string>(COMPONENT_CATEGORIES);

function isComponentCategory(value: string): value is ComponentCategory {
  return COMPONENT_CATEGORY_VALUES.has(value);
}
...
const MatrixRow = memo(function MatrixRow({
  ...
  const hasProps = (component.props?.length ?? 0) > 0;
  const shouldHighlight = highlightUnused === true;
  ...
  return (
    <div>
      <div
        ...
        tabIndex={0}
        className={`flex cursor-pointer items-center gap-3 border-b px-4 py-3 transition-colors hover:bg-slate-50 ${isSelected === true ? 'bg-blue-50' : ''} ${isUnused && shouldHighlight ? 'bg-orange-50' : ''} ${isDeprecated && shouldHighlight ? 'bg-red-50' : ''} `}
        onClick={onSelect}
        onKeyDown={(event) => {
      ...
      >
        {/* Expand button */}
        {(showVariants === true && hasVariants) || (showProps === true && hasProps) ? (
          <button
            onClick={(e) => {
              e.stopPropagation();
      ...
      {expanded && (
        <div className='border-b bg-slate-50 px-4 py-3 text-sm'>
          {/* Props section */}
          {showProps === true && hasProps && component.props !== undefined && (
            <div className='mb-4'>
          ...
          {/* Variants section */}
          {showVariants === true && hasVariants && component.variants !== undefined && (
            <div className='mb-4'>
          ...
          {/* Deprecation message */}
          {isDeprecated && component.deprecationMessage !== undefined && (
...
export const ComponentUsageMatrix = memo(function ComponentUsageMatrix({
  ...
  const filteredComponents = useMemo(() => {
    ...
    return filteredByCategory.filter(
      (comp) =>
        ...
        comp.description?.toLowerCase().includes(query) === true,
    );
  ...
  const categories = COMPONENT_CATEGORIES.filter((category) => componentsByCategory[category].length > 0);
          ...
          <div className='mt-4 flex flex-col gap-3'>
            <div className='flex gap-3'>
              ...
              <Select
                ...
                onValueChange={(value) => {
                  if (value === 'all' || isComponentCategory(value)) {
