...
const SEARCH_FILTERS = new Set<string>(['all', 'title', 'description', 'type']);

function isSearchFilter(value: string): value is SearchFilter {
  return SEARCH_FILTERS.has(value);
}
...
function calculateSearchScore(item: Item, query: string, filter: SearchFilter): number {
  ...
  // Description match
  if ((filter === 'all' || filter === 'description') && item.description !== undefined) {
  ...
  for (const item of items) {
    ...
    if (score > 0) {
      ...
      if (item.title.toLowerCase().includes(lowerQuery)) {
        matchType = 'title';
      } else if (item.description?.toLowerCase().includes(lowerQuery) === true) {
