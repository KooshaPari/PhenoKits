...
function DecompositionTreeItem({
  ...
  const label = ENTITY_LABELS[node.entityType];
    ...
    <div>
      ...
      >
        ...
        <div className='flex items-center gap-1 opacity-0 transition-opacity group-hover:opacity-100'>
          ...
          {node.componentPath !== undefined && onViewInCode !== undefined && (
            <Tooltip delayDuration={200}>
              <TooltipTrigger asChild>
                <Button
                  variant='ghost'
...
function buildDecompositionTree(
  ...
  function buildNode(item: Item, depth: number): DecompositionNode {
    ...
    return {
      ...
      children,
      componentPath: getStringMetadata(metadata, 'componentPath'),
      depth,
      entityType,
  ...
  if (rootId !== undefined) {
    ...
    rootItems = childrenMap.get('root') ?? uiItems.filter((i) => i.parentId === undefined);
  }
