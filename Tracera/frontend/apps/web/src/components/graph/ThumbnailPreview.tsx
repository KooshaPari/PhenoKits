...
function ThumbnailPreviewComponent({
  ...
  const hasDisplayScreenshot = displayScreenshot != null && displayScreenshot.length > 0;
  const hasScreenshotUrl = screenshotUrl != null && screenshotUrl.length > 0;
  const hasCode = componentCode != null && componentCode.length > 0;
  const hasLabel = label != null && label.length > 0;
  const hasVersion = version != null && version.length > 0;

  const content = useMemo(() => {
    if (showFullScreenshot && hasScreenshotUrl) {
      return (
    ...
    if (hasScreenshot != null && hasDisplayScreenshot) {
      return (
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                type='button'
                className='bg-muted/50 group relative w-full cursor-pointer overflow-hidden rounded-lg border-0 p-0'
                onClick={() => {
          ...
          <Tooltip delayDuration={300}>
            ...
            <TooltipContent side='right' className='w-72'>
              <div className='space-y-2'>
                {hasLabel && <p className='font-semibold'>{label}</p>}
                {hasDisplayScreenshot && (
                  <img
                    src={displayScreenshot}
                    alt={label ?? 'Screenshot'}
