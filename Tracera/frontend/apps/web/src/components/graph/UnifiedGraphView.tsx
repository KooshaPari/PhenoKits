...
function filterByPerspective(
  ...
  if (perspective == null || perspective === 'all') {
...
function filterByDimensions(
  ...
  const filteredItems = items.filter((item) =>
    filters.every((filter) => itemMatchesDimensionFilter(item, filter)),
  );
  const itemIds = new Set(filteredItems.map((i) => i.id));
  const filteredLinks = links.filter(
    (link) => itemIds.has(link.sourceId) && itemIds.has(link.targetId),
...
const itemMatchesDimensionFilter = (item: Item, filter: DimensionFilter): boolean => {
  const dimensionValue = getItemDimensionValue(item, filter.dimension);
  ...
  switch (filter.operator) {
    case 'eq': {
      return dimensionValue === filter.value;
    }
    case 'neq': {
      return dimensionValue !== filter.value;
    }
    ...
    case 'in': {
      return Array.isArray(filter.value) && filter.value.some((value) => value === dimensionValue);
    }
    case 'not_in': {
      return Array.isArray(filter.value) && filter.value.every((value) => value !== dimensionValue);
    }
    default: {
      return false;
    }
...
const getItemDimensionValue = (item: Item, dimension: DimensionFilter['dimension']): unknown => {
  ...
  switch (dimension) {
    case 'maturity': {
      return dimensions.maturity;
    }
    case 'complexity': {
      return dimensions.complexity;
    }
    case 'coverage': {
      return dimensions.coverage;
    }
    case 'risk': {
      return dimensions.risk;
    }
    case 'custom': {
      return dimensions.custom;
    }
    default: {
      if (dimensions.custom === undefined) {
        return undefined;
      }
...
function addEquivalenceLinks(
  ...
  if (mode === 'hide' || equivalenceLinks.length === 0) {
    return links;
  }
