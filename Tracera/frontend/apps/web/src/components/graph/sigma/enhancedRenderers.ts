...
const readString = (record: object, key: string): string | undefined => {
...
const enhancedNodeRenderer = (
  ...
  const hovered = Reflect.get(data, 'hovered') === true;
...
const enhancedEdgeRenderer = (
  ...
  const highlighted = Reflect.get(data, 'highlighted') === true;
