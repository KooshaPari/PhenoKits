...
const jsonReplacer = (_key: string, value: unknown): unknown => value;
...
const isUndefinedValue = (value: unknown): boolean => value === undefined;
...
const DiffViewer = ({ fieldChanges, compact = false }: DiffViewerProps): JSX.Element => {
  ...
  const handleCopyValue = useCallback((value: unknown, field: string): void => {
...
const renderValueContent = (value: unknown): ReactNode => {
  ...
  if (typeof value === 'object') {
    return (
      <pre className='whitespace-pre-wrap'>{JSON.stringify(value, jsonReplacer, JSON_INDENT)}</pre>
    );
  }
...
const formatValueForCopy = (value: unknown): string => {
  ...
  if (typeof value === 'object') {
    return JSON.stringify(value, jsonReplacer, JSON_INDENT);
  }
