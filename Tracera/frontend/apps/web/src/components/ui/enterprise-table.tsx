...
function getSearchableValues(value: unknown): unknown[] {
  ...
  React.useEffect(() => {
    ...
    const globalFilter = searchQuery
      ? table
          .getFilteredRowModel()
          .rows.filter((row) => {
            return getSearchableValues(row.original).some((value) =>
...
export function createEnterpriseColumn<TData, TValue>({
  ...
  const column: ColumnDef<TData, TValue> = {
    id,
    header,
    cell: (context: CellContext<TData, TValue>) => cell(context),
  };

  if (size !== undefined) column.size = size;
  if (maxSize !== undefined) column.maxSize = maxSize;
  if (minSize !== undefined) column.minSize = minSize;
  if (enableSorting !== undefined) column.enableSorting = enableSorting;
  if (enableFiltering !== undefined) column.enableColumnFilter = enableFiltering;
  if (enableHiding !== undefined) column.enableHiding = enableHiding;
  if (enableResizing !== undefined) column.enableResizing = enableResizing;
  if (meta !== undefined) {
    column.meta = {
      ...meta,
