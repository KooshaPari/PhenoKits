...
export class MemoryCache implements IObservableCache {
  ...
  get<T = unknown>(key: string): Promise<T | null> {
    ...
    if (isExpired(entry.metadata.expiresAt)) {
      void this.delete(key); // Clean up expired entry
      this.totalMisses++;
      this.emit({
        type: EventType.MISS,
        backend: this.name,
  ...
  async invalidate(options: InvalidateOptions): Promise<number> {
    ...
    for (const [key, entry] of this.entries) {
      ...
      // Match by pattern
      if (options.pattern !== undefined && matchesPattern(key, options.pattern)) {
        shouldDelete = true;
      }

      // Match by tags
      if (options.tags?.some((tag) => entry.metadata.tags.includes(tag)) === true) {
        shouldDelete = true;
      }
  ...
  keys(pattern?: string): Promise<string[]> {
    ...
    if (pattern === undefined || pattern.length === 0) return Promise.resolve(allKeys);
    return Promise.resolve(allKeys.filter((key) => matchesPattern(key, pattern)));
  }
  ...
  private async evictLRU(): Promise<void> {
    ...
    if (keyToEvict === undefined || keyToEvict.length === 0) return;
