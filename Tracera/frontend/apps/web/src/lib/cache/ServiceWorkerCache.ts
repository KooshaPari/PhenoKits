...
const parseTagsHeader = (tagsHeader: string): string[] => {
  try {
    const tags: unknown = JSON.parse(tagsHeader);
...
export class ServiceWorkerCache implements IObservableCache {
  ...
  async get<T = unknown>(key: string): Promise<T | null> {
    try {
      ...
      if (expiresHeader !== null && expiresHeader.length > 0) {
        ...
        if (options.tags !== undefined && options.tags.length > 0) {
          ...
          if (response) {
            ...
            if (tagsHeader !== null && tagsHeader.length > 0) {
              const tags = parseTagsHeader(tagsHeader);
  ...
  async keys(pattern?: string): Promise<string[]> {
    try {
      ...
      if (pattern === undefined || pattern.length === 0) return urls;
   ...
   */
  touch(_key: string): Promise<boolean> {
    ...
    try {
      ...
      const timestampedRequests = await Promise.all(
        requests.map(async (request) => {
          const response = await cache.match(request);
          const timestamp = response?.headers.get('X-Cache-Timestamp') ?? null;
          return {
            request,
            timestamp: timestamp !== null && timestamp.length > 0 ? parseInt(timestamp, 10) : 0,
          };
        }),
      );
