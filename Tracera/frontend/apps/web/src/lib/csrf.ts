...
const isCSRFTokenResponse = (data: unknown): data is CSRFTokenResponse =>
  typeof data === 'object' &&
  data !== null &&
  'token' in data &&
...
export const fetchCSRFToken = async (): Promise<string> => {
  ...
  tokenFetchPromise = (async () => {
    try {
      ...
      if (!isCSRFTokenResponse(data) || data.token.length === 0) {
...
export const extractCSRFTokenFromResponse = (response: Response): string | null => {
  ...
  if (setCookie?.includes(CSRF_COOKIE_NAME) === true) {
...
export const handleCSRFError = async (response: Response): Promise<boolean> => {
  ...
  if (response.status === HTTP_STATUS_FORBIDDEN) {
    ...
    if (contentType?.includes('application/json') === true) {
      try {
        const data: unknown = await response.json();
        const errorMessage = readErrorMessage(data)?.toLowerCase() ?? '';
        const isCsrfError = errorMessage.includes('csrf') || errorMessage.includes('token');
...
export const logCSRFState = (): void => {
  ...
  logger.info('Token in memory:', csrfToken !== null && csrfToken.length > 0 ? 'Yes' : 'No');
  const tokenPreview =
    csrfToken !== null && csrfToken.length > 0 ? `${csrfToken.slice(0, tokenPreviewLength)}...` : 'None';
