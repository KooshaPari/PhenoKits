...
export function aggregateParallelEdges(
  ...
  for (const [key, groupEdges] of edgeGroups) {
    if (groupEdges.length < minEdgesForAggregation) {
      ...
      const dominantType = getDominantType(groupEdges);
...
function getDominantType(edges: EdgeBase[]): LinkType {
  const firstEdge = edges[0];
  if (!firstEdge) {
    throw new Error('Cannot determine dominant type for an empty edge list');
  }
  ...
  for (const edge of edges) {
    const count = (typeCounts.get(edge.type) ?? 0) + 1;
    typeCounts.set(edge.type, count);
    if (count > maxCount) {
      maxCount = count;
      dominantType = edge.type;
    }
  }
