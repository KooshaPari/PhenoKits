...
function isSortBy(value: unknown): value is SortBy {
  return value === 'name' || value === 'items' || value === 'progress';
}

export function DashboardView({ systemStatus }: DashboardViewProps = {}): ReactElement {
  ...
  const mcpBaseUrl = systemStatus?.mcp?.baseUrl ?? null;
  const hasMcpBaseUrl = mcpBaseUrl !== null && mcpBaseUrl.length > 0;
  ...
  useEffect(() => {
    if (projectsArray.length > 0 && pinnedProjectId === null) {
      const firstProject = projectsArray[0];
      if (firstProject?.id !== undefined && firstProject.id.length > 0) {
      ...
      <div className='flex flex-col justify-between gap-4 md:flex-row md:items-center'>
        <div>
          ...
          <div className='mt-3 flex flex-wrap items-center gap-2'>
            ...
            <Badge
              variant={hasMcpBaseUrl ? 'default' : 'secondary'}
              className='text-xs'
            >
              MCP: {hasMcpBaseUrl ? 'connected' : 'not configured'}
            </Badge>
            {hasMcpBaseUrl ? (
              <Button
                variant='outline'
        ...
        {viewMode === 'grid' ? (
          <div className='stagger-children grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'>
            {filteredProjects.map((project) => {
              const handlePin = (e: MouseEvent): void => {
                e.preventDefault();
                e.stopPropagation();
