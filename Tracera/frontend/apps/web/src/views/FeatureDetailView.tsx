...
interface FallbackActivityEntry {
  date: string;
  detail: string;
  label: string;
}

function getRouteParams(params: unknown): RouteParams {
  return typeof params === 'object' && params !== null ? params : {};
}

function getStringParam(params: RouteParams, key: keyof RouteParams): string {
...
export function FeatureDetailView(): ReactElement {
  const params = getRouteParams(useParams({ strict: false }));
  const navigate = useNavigate();
  const featureId = getStringParam(params, 'featureId');
  ...
  const scenarios = useMemo(
    () => (scenariosData === undefined ? EMPTY_SCENARIOS : scenariosData.scenarios),
    [scenariosData],
  );
  ...
  useEffect(() => {
    if (selectedScenarioId.length === 0 && scenarios.length > 0) {
          ...
          {featureActivities && featureActivities.length > 0
            ...
            : ([
                ...
                feature.updatedAt
                  ...
                  : null,
              ].filter((entry): entry is FallbackActivityEntry => entry !== null))
                .map((entry) => (
                  <div
                  ...
                  <div className='text-muted-foreground space-y-3 text-sm'>
                    ...
                    <div className='text-muted-foreground flex items-center justify-between text-xs'>
                      <button
                        ...
                        onClick={() => {
                          setActivityPage(1);
                          setActivityPageSize(scenarioActivityTotal > 0 ? scenarioActivityTotal : 1000);
                        }}
                    ...
                    <div className='text-muted-foreground flex items-center gap-2 text-xs'>
                      <label htmlFor='scenario-activity-rows'>Rows</label>
                      <select
                        id='scenario-activity-rows'
