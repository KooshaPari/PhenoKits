...
import type { Link, TypedItem } from '@tracertm/types';
...
export function ImpactAnalysisView({ projectId }: ImpactAnalysisViewProps): ReactElement {
  ...
  const items: TypedItem[] = itemsData === undefined ? EMPTY_ITEMS : itemsData.items;
  const links: Link[] = linksData === undefined ? EMPTY_LINKS : linksData.links;
  ...
  const analyzeImpact = (itemId: string): ImpactResult => {
    if (links.length === 0) {
  ...
  const impact = selectedItemId === undefined ? undefined : analyzeImpact(selectedItemId);
            ...
            <div className='custom-scrollbar max-h-[60vh] space-y-1 overflow-y-auto pr-2'>
              {filteredItems.map((item) => (
                <button
                  ...
                  className={cn(
                    'p-3 rounded-xl cursor-pointer transition-all border-2 border-transparent group text-left w-full',
