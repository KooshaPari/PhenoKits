...
**tests** — Central repository for cross-cutting test suites and integration tests

## Project Summary

This project provides end-to-end system reliability, cross-project integration validation, and comprehensive test coverage for shared concerns.
...
| Package Manager | Multi | Latest |
| Linter | Multi | Latest |
| Test Runner | Multi | Latest |

## Key Commands

```bash
# Development
(per-suite)              # Install dependencies
(per-suite)                 # Run tests
(per-suite)                 # Lint
(per-suite)               # Format
(per-suite)                 # Type check

# Build
(per-suite)                # Build package
```
