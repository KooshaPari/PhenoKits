...
## 2026-04-24 — Archive contents audit & code extraction mapping

**Scope:** 29 archived repositories (.archive/) audited for reuse viability, metadata consistency, and extractable code patterns.

**Audit Summary:**
- ✅ **DEPRECATION.md consistency:** 14 archives added (kitty-specs, koosha-portfolio, KWatch, PhenoProject, phenoSDK-deprecated-2026-04-05, phenotype-config-ts, phenotype-docs-engine, phenotype-gauge, phenotype-middleware-py, phenotype-nexus, phenotype-types, phenotype-vessel, RIP-Fitness-App, Tossy). All 29 archives now have restore commands.
- ✅ **Total archived LOC:** ~1.1M (src only: Rust, Go, Python, TypeScript, Java)
- ⚠️ **High-LOC archives (extraction candidates):** 6 repos with >30K LOC; 8 extractable patterns identified

**Extractable Code Patterns:**

| Archive | LOC | Extractables | Migration Path |
|---------|-----|--------------|-----------------|
| **phenodocs** | 281K | VitePress plugins (Markdown transformers, sidebar auto-gen, search indexing, API doc generation) | `@phenotype/vitepress-plugins` if visualization subsystem scales |
| **PhenoLang-actual** | 215K | Compiler core: AST types, type inference, LLVM/Cranelift codegen adapters, optimization passes | `phenotype-compiler-core`; backends as plugins |
| **canvasApp** | 442K | WebGL/Canvas primitives, GPU buffer mgmt, shader compilation, mesh utilities | `@phenotype/canvas-core` if visualization needed |
| **phenoEvaluation** | 81K | Pytest plugin (metrics collection, test result tracking), benchmark framework (statistical analysis, regression detection), coverage aggregation | `@phenotype/pytest-phenotype` + metrics to `phenotype-metrics` |
| **phenoSDK-deprecated-2026-04-05** | 88K | API versioning patterns (semver routing, deprecation handling), client generation (TypeScript/Go/Python stubs), rate limiting + retry logic | Keep archived as SDK reference; patterns valuable for new SDK work |
| **pgai** | 36K | PostgreSQL extensions, vector embedding adapters (pgvector integration), query optimizer hooks, semantic search index strategies | Low reuse outside PG ecosystem; keep archived |

**Re-activation Candidates:** None. All archives remain in cold storage. No evidence of active development needs.
